...
Возвращает количество применённых блоков. Требует подтверждения пользователя как обычный write — diff отображается в том же модальном окне.`,
  ...
  },
  {
    ...
    description: 'Запускает несколько подзадач параллельно. Каждая задача — независимый sub-agent С ИНСТРУМЕНТАМИ (agent-loop): он сам читает файлы / ищет по проекту, а с ролью executor/verifier правит код и запускает проверки. Provider задаётся НА КАЖДУЮ задачу — в одном батче можно СМЕШИВАТЬ разные провайдеры (например API и CLI). Поддерживает роли (planner/critic/executor/verifier/researcher) — роль определяет и поведение, и whitelist инструментов. Субагенты НЕ могут делегировать дальше (защита от рекурсии). Используй для параллельного анализа, кросс-верификации, исследования нескольких зон. До 10-12 задач с контролем concurrency.',
    parameters: {
      type: 'object',
      properties: {
        tasks: {
