...
const ENTRIES = [
  {
    ...
    deployed: '20.06.2026',
    treeVersion: '1.5.17',
    title: 'Фикс автообновления: staged install через проверенный node apply',
    changes: [
      'Причина повторной ошибки: payload на диске был валидный (app.asar 236 МБ), но отдельная кнопочная watchdog-ветка теряла metadata и не давала надёжной диагностики.',
      'Staged install больше не использует отдельный PowerShell robocopy-скрипт — теперь идёт через тот же node apply-silent-update.cjs --payload-root, который проверен вручную.',
      'Добавлен лог установки %LOCALAPPDATA%\\verstak-update-install.log: путь node, args, stdout/stderr и exit code.',
    ],
  },
  {
    commit: '61e6ccb+',
    deployed: '20.06.2026',
    treeVersion: '1.5.17',
    title: 'Обновления: очистка временных payload и битого staging',
    changes: [
      'Старые временные папки распаковки Verstak в Temp автоматически удаляются при старте и после staging/установки.',
      'При неудачном staging или установке битый payload удаляется сразу, чтобы не копиться и не предлагаться повторно.',
      'Watchdog staged-install чистит staging при ошибке; успешная установка удаляет staging после копирования.',
    ],
  },
  {
    commit: '61e6ccb+',
    deployed: '20.06.2026',
    treeVersion: '1.5.17',
    title: 'Фикс автообновления: защита установки от пустого app.asar',
    changes: [
      'Watchdog установки проверяет staged payload перед robocopy и установленную папку после robocopy.',
      'Единый порог валидности app.asar ≥10 МБ добавлен в updater payload extractor и installer extractor.',
      'Тесты покрывают пустой и микроскопический app.asar; staging-интеграция подтверждает распаковку 235+ МБ.',
    ],
  },
  {
    commit: '61e6ccb+',
    deployed: '20.06.2026',
    treeVersion: '1.5.17',
    title: 'Фикс staging: пустой app.asar после распаковки',
    changes: [
      'poll завершает staging только при валидном payload (≥10 МБ app.asar), не по одному progress 100%.',
      '7za в node fallback: spawnSync вместо spawn (Job Object Electron → 0 байт).',
      'PS: manifest до progress 100%; минимальный размер app.asar в Test-PayloadRoot.',
    ],
  },
  {
    commit: '61e6ccb+',
    deployed: '20.06.2026',
    treeVersion: '1.5.17',
    title: 'Фикс staging: зависание на 100% «завершена» (деплой)',
    changes: [
      'poll завершает staging сразу при step done на диске — не ждёт exit-файл/процесс.',
      'PS: без раннего return — wrapper всегда пишет stage-*.exit.',
      'Таймер recovery 1,5 с + get-state поднимает ready при валидном payload.',
    ],
  },
  {
    commit: '61e6ccb+',
    deployed: '20.06.2026',
    treeVersion: '1.5.17',
    title: 'Скачивание: быстрый путь electron-updater + оптимизация fallback',
    changes: [
      'electron-updater снова основной: force download, если probe новее, а check вернул not available.',
      'Fallback: pipeline 2 МБ буфер, sha512 на лету (без второго прохода по 360 МБ), UI % раз в 300 мс.',
    ],
  },
  {
    commit: '61e6ccb+',
    deployed: '20.06.2026',
    treeVersion: '1.5.17',
    title: 'Фикс скачивания: fallback без electron-updater',
    changes: [
      '«Please check update first» — downloadUpdate без успешного checkForUpdates; теперь проверяем isUpdateAvailable.',
      'Fallback: прямое скачивание установщика с GitHub Releases с % и sha512.',
      'Английская ошибка больше не показывается пользователю.',
    ],
  },
  {
    commit: '61e6ccb+',
    deployed: '20.06.2026',
    treeVersion: '1.5.17',
    title: 'Обновления: проценты подготовки staging',
    changes: [
      'Подготовка в фоне: реальный % по этапам (установщик → приложение → проверка), не крутящийся ползунок.',
      'Оценка по размеру app.asar при распаковке; подсказка «обычно 1–3 минуты».',
      'stage-progress.json + опрос из Electron; тот же прогресс в node fallback.',
    ],
  },
  {
    commit: '61e6ccb+',
    deployed: '20.06.2026',
    treeVersion: '1.5.17',
    title: 'Обновления: прогресс-бар и единый список релизов',
    changes: [
      'В «Проверить обновления» — бар скачивания (%, МБ) и индикатор подготовки staging.',
      'Убрана кнопка «Посмотреть текущий список» — осталось «Все обновления» с текущей версией и меткой.',
    ],
  },
  {
    commit: '61e6ccb+',
    deployed: '20.06.2026',
    treeVersion: '1.5.17',
    title: 'Обновления: убрана кнопка «Очистить кэш»',
    changes: [
      'Кнопка сносила staging и ломала готовое обновление — удалена из настроек.',
      'IPC update:clear-cache и resetUpdaterSession убраны.',
      'Подсказка в настройках обновлений без отсылки к очистке кэша.',
    ],
  },
  {
    commit: '61e6ccb+',
    deployed: '20.06.2026',
    treeVersion: '1.5.17',
    title: 'Фикс staging: не сносим готовый payload',
    changes: [
      'clearStagingIfWrongVersion больше не удаляет валидный staging 1.5.20 при probe на 1.5.19.',
      'PS-скрипт: если app.asar уже целый — только manifest, без Remove-Item всего каталога.',
      'reconcileStagingManifest не подменяет версию чужого payload.',
      'PowerShell staging через cmd start /wait — вне Job Object Electron; retry не чистит целый payload.',
      'При старте: восстановление ready из error, если staging на диске валиден.',
    ],
  },
  {
    commit: '61e6ccb',
    deployed: '20.06.2026',
    treeVersion: '1.5.17',
    title: 'Обновления: main 1.5.20 впереди — ставим последний Release',
    changes: [
      'package.json на main (1.5.20) без установщика больше не блокирует апдейт до 1.5.19.',
      'fetchLatestPublishedReleaseVersion: latest.yml + fallback по URL releases/latest.',
      'resolveInstallableUpdate: выбирает последнюю версию с артефактами, pending только если нет ни одного установщика.',
    ],
  },
  {
    commit: '61e6ccb',
    deployed: '20.06.2026',
    treeVersion: '1.5.17',
    title: 'Фикс staging: BOM manifest + Job Object Electron',
    changes: [
      '7za/node только через PowerShell Start-Process — spawnSync из Verstak.exe давал app.asar 0 байт.',
      'manifest.json: UTF-8 без BOM (Set-Content ломал JSON.parse → ложная «пустой app.asar»).',
      'reconcileStagingManifest: payload на диске цел — восстанавливаем ready даже при битом manifest.',
      'Тесты: update-staging.test.ts, test-staging-integration, test-electron-7za-child, test-electron-ps-stage.',
    ],
  },
  {
    commit: '61e6ccb',
    deployed: '20.06.2026',
    treeVersion: '1.5.17',
    title: 'Обновления: фоновая предустановка + плашка «Готово»',
    changes: [
      'Скачивание и распаковка payload в фоне — приложение не закрывается и не блокируется.',
      'После предустановки — плашка в правом нижнем углу: «Позже» / «Установить».',
      '«Позже» скрывает до перезапуска; при новом запуске плашка снова, если обновление готово.',
      '«Установить» — закрытие, robocopy из staged payload, автоперезапуск.',
      'update-staging.ts, apply-silent-update.cjs --stage-only / --payload-root, UpdateReadyToast.',
    ],
  },
  {
    commit: '61e6ccb',
    deployed: '20.06.2026',
    treeVersion: '1.5.17',
    title: 'Фикс тихой установки обновлений',
    changes: [
      'Убрана автоустановка после скачивания — закрытие и установка только по кнопке «Установить обновление».',
      'Watchdog: apply-silent-update.cjs через ELECTRON_RUN_AS_NODE (был баг Find-FileRecursive в PowerShell — установка молча падала).',
      'ensureDownload при старте модалки; повторная проверка артефактов для pending-релиза.',
      'Тексты UI: «скачана — нажмите Установить», без «устанавливаем в фоне».',
    ],
  },
  {
    commit: '61e6ccb',
    deployed: '19.06.2026 16:53',
    treeVersion: '1.5.17',
    title: 'Локальный деплой upstream 1.5.17 (обход зависшего установщика)',
    changes: [
      'fast-forward origin/main → 61e6ccb (upstream 1.5.17).',
      'Сборка dist:win + deploy:local напрямую в %LOCALAPPDATA%\\Programs\\Verstak — без Verstak-Setup.',
      'Причина: встроенный установщик зависал на «Загрузка мастера установки».',
    ],
  },
  {
    version: '1.5.17',
    build: '19.06.2026',
    deployed: '19.06.2026',
    title: 'Дизайн + надёжное обновление: таймер ответа, читаемый код, тихая установка',
    changes: [
      'Пока агент отвечает — в шапке сообщения видно «выполняется N с» (тикает каждую секунду), после завершения — «за N с». Работает в проектах, ветках и справке; длительность фиксируется и при остановке/ошибке.',
      'Текст внутри блоков кода в markdown снова читаемый (контраст починен).',
      'Обновление стало надёжным: установщик больше не падает на 9% — пакет распаковывается в рантайме, тихая установка без сбоев. Окно «Что нового» после апдейта показывает изменения из встроенного списка (не зависит от лимитов GitHub). Если нативный модуль БД ломается после обновления — авто-починка при старте.',
    ],
  },
  {
    version: '1.5.16',
    build: '19.06.2026',
    deployed: '19.06.2026',
    title: 'Pipeline: задача с доказательством — от брифа до Proof Pack',
    changes: [
      'Новый сценарий «▶ Pipeline» в композере: один сквозной поток от формулировки задачи до проверяемого доказательства, что работа сделана. Бриф (цель / границы / как проверим) → план (только чтение, файлы не трогает) → выполнение → проверка → Proof Pack.',
      'На каждом шаге видно, где ты: баннер «Pipeline · N/5». Шаг «Проверка» показывает реальный статус (зелёный — прошло, жёлтый — не подтверждено, красный — упало) и даёт дожать проверку. Финал — HTML Proof Pack одной кнопкой (diff + проверки + стоимость), открывается прямо в окне.',
      'Pipeline доступен для API-провайдеров (полный контроль и доказательство). На CLI-провайдере кнопка подсказывает переключиться. Новым пользователям — кнопка «Попробовать Pipeline» в онбординге с готовым примером задачи.',
    ],
  },
  {
    deployed: '19.06.2026',
  ...
  },
  {
    ...
    build: '19.06.2026',
    deployed: '19.06.2026',
    title: 'Справка в приложении, черновики композера, вкладка «Файлы», умное автообновление',
    changes: [
  ...
  },
  {
    ...
    deployed: '18.06.2026',
    treeVersion: '1.5.5',
    title: 'Черновики композера по проектам и веткам',
    changes: [
      'Несохранённый текст и вложения остаются в своём чате при переключении проекта или ветки.',
      'Отдельный черновик для справки «?»; после отправки композер очищается.',
      'Черновики в памяти до выхода из приложения; при удалении проекта — чистка его ключей.',
    ],
  },
  {
    commit: 'bce8d3b',
  ...
  },
  {
    ...
    deployed: '18.06.2026',
    treeVersion: '1.5.5',
    title: 'Справка: отдельные уведомления',
    changes: [
      'Тост при ответе в режиме справки: «Режим справки — ответ получен», без имени проекта.',
      'Клик по уведомлению открывает чат справки, не рабочий чат проекта.',
      'В шапке чата справки — ❓ Справка Verstak вместо папки проекта.',
    ],
  },
  {
    commit: 'b698298',
  ...
  },
  {
    ...
    deployed: '18.06.2026',
    treeVersion: '1.5.5',
    title: 'Сайдбар, журнал, очередь в чате',
    changes: [
      'Навигация: Работа / Контроль / Проект / Инструменты; «Ещё» убран; Чеклист, Прогоны, Файлы в своих блоках.',
      'Журнал: компактные фильтры, редактирование своих заметок; голубая подсказка очереди со стрелкой вправо.',
      'Вкладка «Файлы» — дерево проекта в основной области.',
    ],
  },
  {
    commit: '27bb96b',
  ...
  },
  {
    ...
    deployed: '17.06.2026',
    treeVersion: '1.5.5',
    title: 'Обновления: «Проверяю…» больше не висит',
    changes: [
      'Готовый pending-кэш (1.5.7) больше не перечитывается на sha512 — мгновенное «Готово к установке».',
      'Таймауты checkForUpdates / runCheck; выход из phase checking если updater молчит.',
      'Кнопка «Проверить» корректно показывает ready/downloading после IPC.',
    ],
  },
  {
    commit: '15be0c6',
