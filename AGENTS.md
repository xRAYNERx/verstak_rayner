# AGENTS.md — регламент проекта Verstak

Файл читается user_layer'ом Verstak и Codex как «правила работы с этим проектом». См. `electron/ai/user-layer.ts` — порядок поиска: AGENTS.md → AGENTS.md → GEMINI.md → .verstak/RULES.md.
...
**Базовые фичи:**
- **8 провайдеров одновременно:** Gemini API / Gemini CLI / Codex API / Codex / Grok API / Grok Build / OpenAI API / Codex CLI. Аккаунт упал — переключился, не теряя работу.
...
**V3 фичи:**
- **Skills как first-class.** Frontmatter `.md` файлы → system prompt + tools_allow + context_loaders + default_provider/model. Авто-импорт из `~/.Codex/skills/` + `~/.verstak/skills/` + 3 built-in (code-review / git-summary / explain-code). Picker 🎭 в composer + slash commands `/code-review`.
...
- **Codex OAuth env-passthrough** — Settings field, Verstak передаёт `CLAUDE_CODE_OAUTH_TOKEN` дочернему Codex процессу, headless+Max заработал.
...
│   ├── gemini.ts, Codex.ts, grok.ts, openai.ts  ← API-провайдеры
│   ├── *-cli.ts             ← CLI-провайдеры (Codex и т.п.)
...
│   ├── user-layer.ts        ← поиск AGENTS/Codex/GEMINI.md/RULES
...
- **Новый AI-провайдер:** `electron/ai/{name}.ts` + регистрация в `registry.ts`. Если это API — реализуй `ChatProvider.send` как async generator. Если CLI — посмотри `Codex-cli.ts` как шаблон (treeKill + stdin payload + stream-json parser).
...
- **Новый skill:** просто `.md` файл в `~/.verstak/skills/` (или редактируй `~/.Codex/skills/` — авто-импортится). Frontmatter: id (обязательно) + name/description/icon/slash/tools_allow/context_loaders/suggested_prompts. Body = system prompt. Для built-in (захардкоженного fallback) — `electron/ai/skills/built-in.ts`.
