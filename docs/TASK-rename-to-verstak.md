# TASK: Переименование проекта geminigrok → verstak
...
Продукт **переименовывается** с рабочего имени `GeminiGrok` (от 3 моделей) на финальное **`Verstak`** (русский «верстак» — рабочее место мастера). Новое имя отражает суть: open IDE для любых AI-моделей, vendor-agnostic, РФ-first.

Бренд-брифинг с палитрой, шрифтами, USP — `C:\Users\Pavel\Downloads\verstak_brand_brief_2026-05-25.html`.
Готовый лендинг — `C:\Users\Pavel\geminigrok\landings\verstak\`.
Готовый логотип (PNG в наборе размеров + SVG) — `C:\Users\Pavel\geminigrok\landings\verstak\assets\`.
...
| Найти | Заменить на |
|---|---|
| `geminigrok` (lowercase) | `verstak` |
| `GeminiGrok` (CamelCase) | `Verstak` |
| `GEMINIGROK` (UPPER) | `VERSTAK` |
| `gemini-grok` (kebab) | `verstak` |
| `Gemini Grok` (с пробелом) | `Verstak` |
| `gemini_grok` (snake) | `verstak` |
| `com.pavelfrolof.geminigrok` | `ru.verstak.ide` |
| `.geminigrok` (папка-конфиг) | `.verstak` |
...
### Файлы где есть `geminigrok` (29 шт):
...
scripts/launch.bat
docs/dev-journal.md
docs/superpowers/plans/2026-05-19-geminigrok-mvp.md
docs/superpowers/specs/2026-05-19-geminigrok-design.md
.gitignore
CLAUDE.md
DEVLOG.md
README.md
...
### Файлы где есть `GeminiGrok` (27 шт):
...
.geminigrok/RULES.md
index.html
docs/superpowers/specs/2026-05-23-geminigrok-v3-vision.md
```
...
+ "description": "Verstak — desktop AI coding IDE, vendor-agnostic, with Russian models. Многомодельный IDE для разработчиков.",
  ...
  "build": {
-   "appId": "com.pavelfrolof.geminigrok",
+   "appId": "ru.verstak.ide",
-   "productName": "GeminiGrok",
+   "productName": "Verstak",
    "copyright": "Copyright © 2026 Pavel Frolov",
    ...
    "nsis": {
      ...
-     "shortcutName": "GeminiGrok",
+     "shortcutName": "Verstak",
-     "artifactName": "GeminiGrok-Setup-${version}-${arch}.${ext}"
+     "artifactName": "Verstak-Setup-${version}-${arch}.${ext}"
    },
    "portable": {
-     "artifactName": "GeminiGrok-Portable-${version}-${arch}.${ext}"
...
**Текущая**: `C:\Users\Pavel\geminigrok\`
...
3. Переименовать папку:
   ```bash
   mv C:/Users/Pavel/geminigrok C:/Users/Pavel/verstak
   ```
...
6. Поискать абсолютные пути `C:\Users\Pavel\geminigrok` в коде/конфигах — если есть, заменить:
   ```bash
   grep -r "C:\\\\Users\\\\Pavel\\\\geminigrok" . --include="*.ts" --include="*.js" --include="*.json"
   grep -r "C:/Users/Pavel/geminigrok" . --include="*.ts" --include="*.js" --include="*.json"
   ```
...
## 5. ПЕРЕИМЕНОВАНИЕ СКРЫТОЙ ПАПКИ `.geminigrok` → `.verstak`
...
**Шаги**:
1. Переименовать **внутреннюю** папку проекта `C:\Users\Pavel\geminigrok\.geminigrok\` → `.verstak\`
2. Найти в коде все упоминания строки `.geminigrok` и заменить на `.verstak`:
   - `electron/ai/skills/loader.ts`
   - `electron/ai/skills/types.ts`
   - `electron/ai/system-layer.ts`
   - возможно `electron/ai/user-layer.ts`
3. **Миграция для существующих юзеров**: пока продукт не выпущен в публику — миграция не нужна. Если уже есть пара тестовых установок, можно добавить fallback: при старте проекта, если есть `.geminigrok/` но нет `.verstak/` — переименовать.

---

## 6. ОНБОРДИНГ / UI КОПИРАЙТ

Поискать и обновить **тексты в UI**, где могут быть упоминания GeminiGrok:

- `src/components/OnboardingWizard.tsx` — приветственное окно при первом запуске («Welcome to GeminiGrok» → «Welcome to Verstak»)
...
**Запустить grep на каждом** — увидеть контекст, проверить нет ли где «представляйся как GeminiGrok» или подобного в промптах:
```bash
grep -n "GeminiGrok" src/components/*.tsx electron/**/*.ts
```
...
## [1.1.0] — 2026-05-XX
### Changed
- Product rebranded from "GeminiGrok" to "Verstak"
- New logo, icons, app identity (com.pavelfrolof.geminigrok → ru.verstak.ide)
- Config folder `.geminigrok/` → `.verstak/` (auto-migration on first open)
```
...
3. ✅ **Массовая текстовая замена** по всем файлам (см. таблицу из п.1). Можно через sed/regex:
   ...
   grep -rl "geminigrok" --exclude-dir=node_modules --exclude-dir=.git --exclude-dir=out --exclude-dir=release . | xargs sed -i.bak 's/geminigrok/verstak/g'
   grep -rl "GeminiGrok" --exclude-dir=node_modules --exclude-dir=.git --exclude-dir=out --exclude-dir=release . | xargs sed -i.bak 's/GeminiGrok/Verstak/g'
   # Удалить .bak файлы
   find . -name "*.bak" -not -path "*/node_modules/*" -delete
   ```
4. ✅ **Переименовать `.geminigrok/` → `.verstak/`** в корне проекта
...
8. ✅ **Production билд** (для Windows как первый таргет):
   ...
   ```
9. ✅ **Переименовать корневую папку** `geminigrok` → `verstak` (см. п.4)
10. ✅ **Git commit + tag**:
    ```bash
    git add -A
    git commit -m "rebrand: GeminiGrok → Verstak (logo, icons, appId, all references)"
...
- [ ] При новом проекте создаётся папка `.verstak/`, не `.geminigrok/`
- [ ] `npm run dist:win` собирает `Verstak-Setup-1.1.0-x64.exe` с правильной иконкой и именем приложения
- [ ] Поиск `grep -ri "geminigrok\|GeminiGrok" --exclude-dir=node_modules --exclude-dir=.git --exclude-dir=out --exclude-dir=release` возвращает **0 совпадений** (либо только в исторических changelog/docs где упоминание оправдано)
...
| Риск | Митигация |
|---|---|
| Существующая установка пользователя ломается из-за смены `.geminigrok` → `.verstak` | На текущей MVP-стадии пользователей публично нет, риск нулевой. Если нужно — добавить fallback миграцию в `loader.ts` |
| Кто-то из тестеров уже запустил с appId `com.pavelfrolof.geminigrok` — Windows покажет 2 приложения | Уведомить тестеров, удалить старое вручную |
